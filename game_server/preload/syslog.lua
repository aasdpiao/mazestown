...
local function write (priority, fmt, ...)
	...
	if priority >= level then
		local server_str = SERVICE_NAME and "["..SERVICE_NAME.."]" or ""
		skynet.error (server_str .. syslog.prefix[priority].. fmt, ...)
	end
end
...
local function writef (priority, ...)
	if priority >= level then
		local server_str = SERVICE_NAME and "["..SERVICE_NAME.."]" or ""
		skynet.error (server_str .. syslog.prefix[priority]..string.format (...))
	end
end
...
function syslog.setlevel(value)
	level = value
end
